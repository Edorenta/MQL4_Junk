...
int start() {
    ...
    for (i = 0; i < OrdersHistoryTotal(); i++) {
        ...
        if (WinCount_b1 >= max_nb || LossCount_b1 >= max_nb) {
            LossCount_b1 = 0;
            WinCount_b1 = 0;
        }
        //--- short loss counter         
        ...
        if (WinCount_s1 >= max_nb || LossCount_s1 >= max_nb) {
            LossCount_s1 = 0;
            WinCount_s1 = 0;
        };
        //--- long loss counter 2
        ...
        if (WinCount_b2 >= max_nb || LossCount_b2 >= max_nb) {
            LossCount_b2 = 0;
            WinCount_b2 = 0;
        }
        ...
        if (WinCount_s2 >= max_nb || LossCount_s2 >= max_nb) {
            LossCount_s2 = 0;
            WinCount_s2 = 0;
        };
    }
    ...
    case mart:
        if ((OrdersHistoryTotal() != 0)) {
            m_lots_b1 = NormalizeDouble(lots * (MathPow(multiplier, (LossCount_b1 + 1))), Digits);
    ...
    case r_mart:
        if ((OrdersHistoryTotal() != 0)) {
            m_lots_b1 = NormalizeDouble(lots * (MathPow(multiplier, (WinCount_b1 + 1))), Digits);
    ...
    case scale_in_p:
        if ((OrdersHistoryTotal() != 0)) {
            m_lots_b1 = lots + (increment * WinCount_b1);
    ...
    case scale_in_l:
        if ((OrdersHistoryTotal() != 0)) {
            m_lots_b1 = lots + (increment * LossCount_b1);
    ...
    if (hard_stop == false) {
        stoploss_b1 = NormalizeDouble(Ask - (MID - ENV2), Digits);
    ...
    if (hard_stop == true) {
        stoploss_b1 = NormalizeDouble(Ask - (SL * Point), Digits);
    ...
    if (OrdersTotal() < max_trades_total && trade) {
        if (side_kick == true) {
            if (from_bot) {
                if (cycle_buy == 0) {
                    OrderSend(Symbol(), OP_BUY, m_lots_b2, Ask, 0, stoploss_b2, takeprofit_b2, "SideKick - From Bot", magic_b2, 0, Turquoise);
                };
                if (cycle_buy == -1) {
                    OrderSend(Symbol(), OP_BUY, m_lots_s2, Ask, 0, stoploss_b2, takeprofit_b2, "SideKick - From Bot", magic_b2, 0, Turquoise);
                };
                Print(LossCount_b2, LossCount_b1);
            }
            if (from_top) {
                if (cycle_sell == 0) {
                    OrderSend(Symbol(), OP_SELL, m_lots_s2, Bid, 0, stoploss_s2, takeprofit_s2, "SideKick - From Top", magic_s2, 0, Magenta);
                };
                if (cycle_sell == -1) {
                    OrderSend(Symbol(), OP_SELL, m_lots_b2, Bid, 0, stoploss_s2, takeprofit_s2, "SideKick - From Top", magic_s2, 0, Magenta);
                };
                Print(LossCount_s2, LossCount_s1);
            }
            if (to_top) {
                if (cycle_buy == 0) {
                    OrderSend(Symbol(), OP_BUY, m_lots_b2, Ask, 0, stoploss_b2, takeprofit_b2, "SideKick - Cross Top", magic_b2, 0, Turquoise);
                };
                if (cycle_buy == -1) {
                    OrderSend(Symbol(), OP_BUY, m_lots_s2, Ask, 0, stoploss_b2, takeprofit_b2, "SideKick - Cross Top", magic_b2, 0, Turquoise);
                };
                Print(LossCount_b2, LossCount_b1);
            }
            if (to_bot) {
                if (cycle_sell == 0) {
                    OrderSend(Symbol(), OP_SELL, m_lots_s2, Ask, 0, stoploss_s2, takeprofit_s2, "SideKick - Cross Bot", magic_s2, 0, Turquoise);
                };
                if (cycle_sell == -1) {
                    OrderSend(Symbol(), OP_SELL, m_lots_b2, Ask, 0, stoploss_s2, takeprofit_s2, "SideKick - Cross Bot", magic_s2, 0, Turquoise);
                };
                Print(LossCount_s2, LossCount_s1);
            }
        }
        if (middle_kick == true) {
            if (in_middle) {
                if (cycle_buy == 0) {
                    OrderSend(Symbol(), OP_BUY, m_lots_b1, Ask, 0, stoploss_b1, takeprofit_b1, "SideKick - Mid long swing", magic_b1, 0, Turquoise);
                };
                if (cycle_sell == 0) {
                    OrderSend(Symbol(), OP_SELL, m_lots_s1, Bid, 0, stoploss_s1, takeprofit_s1, "SideKick - Mid short swing", magic_s1, 0, Magenta);
                };
                if (cycle_buy == -1) {
                    OrderSend(Symbol(), OP_BUY, m_lots_s1, Ask, 0, stoploss_b1, takeprofit_b1, "SideKick - Mid long swing", magic_b1, 0, Turquoise);
                };
                if (cycle_sell == -1) {
                    OrderSend(Symbol(), OP_SELL, m_lots_b1, Bid, 0, stoploss_s1, takeprofit_s1, "SideKick - Mid short swing", magic_s1, 0, Magenta);
                };
            }
        }
    }
